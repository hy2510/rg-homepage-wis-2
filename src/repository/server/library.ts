...
async function exportVocabularyUrl(
  token: string,
  input: {
  ...
  const request = makeRequest({
    token,
    path: getPath('export/vocabulary'),
    option: {
      method: 'get',
      queryString: {
        levelRoundIds: input.levelRoundIds,
...
async function exportBookListDownloadUrl(
  token: string,
  input: {
    levelRoundIds: string
  },
