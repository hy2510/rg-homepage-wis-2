		...
		if (isMobile) {
			...
			if (md.tablet()) {
				if (isPortrait) {
					...
					$(correctionItemLayout).css('transform', `scale(${wrapperScalePortrait}) translate(-50%, -50%)`);
				} else {
					$(targetLayout).css('transform', `scale(${wrapperScalePortrait})`);
					$(targetLayoutPopStart).css('transform', `scale(${wrapperScalePortrait}) translate(-50%, -50%)`);
					$(targetLayoutPopLevel).css('transform', `scale(${wrapperScalePortrait}) translate(-50%, -50%)`);
			...
			// 모바일일 때
				...
				if (isPortrait) {
					...
					$(correctionItemLayout).css('transform', `scale(${wrapperScaleLandscape}) translate(-50%, -50%)`);
				}
			}
		} else {
			if (screenWidth < 1440) {
				...
				switch(window.orientation) {
					...
					case 0:
						if (isPortrait) {
							...
							$(correctionItemLayout).css('transform', `scale(${wrapperScalePortraitToLandscapeByTab}) translate(-50%, -50%)`);
						} else {
							...
							$(correctionItemLayout).css('transform', `scale(${wrapperScaleLandscapeToPortraitByTab}) translate(-50%, -50%)`);
						}
						break;
					// onload 가로 -> 세로
					case 90:
				...
				switch(window.orientation) {
					...
					case 0:
						if (isPortrait) {
							...
							$(correctionItemLayout).css('transform', `scale(${wrapperScalePortraitToLandscape}) translate(-50%, -50%)`);
						}
						break;
					// onload 가로 -> 세로
					case 90:
					case -90:
						if (isPortrait) {
							...
							$(correctionItemLayout).css('transform', `scale(${wrapperScaleLandscapeToPortrait}) translate(-50%, -50%)`);
						} else {
							$(targetLayout).css('transform', `scale(${wrapperScaleLandscape})`);
							$(targetLayoutPopStart).css('transform', `scale(${wrapperScaleLandscape}) translate(-50%, -50%)`);
							$(targetLayoutPopLevel).css('transform', `scale(${wrapperScaleLandscape}) translate(-50%, -50%)`);
